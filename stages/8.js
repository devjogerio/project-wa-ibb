...
function execute(user, msg) {
	...
	let _search = gestor[pms.p]._produto._searchAnuncio
	let _find = ''
	let _term = ''
	let _found = false
	let _count = 0
	dados[user]._search_anuncio = []
	for (let x in _search) {
		...
		if (
			...
			dados[user]._search_anuncio.push([
				_search[x][0],
				_search[x][1],
				_search[x][2],
				_search[x][3],
	...
	if (_found) {
		_msgRetorno = [
			`💬 ` +
				' _Encontrei ' +
				_count +
				' anúncio(s) com o termo *' +
				msg +
				'* que você digitou._',
		]
	} else {
		_msgRetorno = [`💬 ` + 'Nenhum anúncio encontrado!, tente novamente. ']
	}
	return _msgRetorno
}
